...
public typealias Completion = (_ result: Result<Moya_Hi.Response, MoyaError>) -> Void
...
public protocol MoyaProviderType: AnyObject {
    ...
    func request(_ target: Target, callbackQueue: DispatchQueue?, progress: Moya_Hi.ProgressBlock?, completion: @escaping Moya_Hi.Completion) -> Cancellable
}
...
open class MoyaProvider<Target: TargetType>: MoyaProviderType {
    ...
    public typealias StubClosure = (Target) -> Moya_Hi.StubBehavior
    ...
    open var inflightRequests: [Endpoint: [Moya_Hi.Completion]] { internalInflightRequests }

    @Atomic
    var internalInflightRequests: [Endpoint: [Moya_Hi.Completion]] = [:]
    ...
    @discardableResult
    open func stubRequest(_ target: Target, request: URLRequest, callbackQueue: DispatchQueue?, completion: @escaping Moya_Hi.Completion, endpoint: Endpoint, stubBehavior: Moya_Hi.StubBehavior) -> CancellableToken {
        let callbackQueue = callbackQueue ?? self.callbackQueue
...
public extension MoyaProvider {
    ...
    /// Do not stub.
    final class func neverStub(_: Target) -> Moya_Hi.StubBehavior { .never }

    /// Return a response immediately.
    final class func immediatelyStub(_: Target) -> Moya_Hi.StubBehavior { .immediate }

    /// Return a response after a delay.
    final class func delayedStub(_ seconds: TimeInterval) -> (Target) -> Moya_Hi.StubBehavior {
        return { _ in .delayed(seconds: seconds) }
    }
}

/// A public function responsible for converting the result of a `URLRequest` to a Result<Moya_Hi.Response, MoyaError>.
public func convertResponseToResult(_ response: HTTPURLResponse?, request: URLRequest?, data: Data?, error: Swift.Error?) ->
    Result<Moya_Hi.Response, MoyaError> {
        switch (response, data, error) {
        case let (.some(response), data, .none):
            let response = Moya_Hi.Response(statusCode: response.statusCode, data: data ?? Data(), request: request, response: response)
